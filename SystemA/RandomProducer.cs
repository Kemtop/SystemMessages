...
namespace SystemA
{
    ...
    /// </summary>
    class RandomProducer : IRandomProducer,IDisposable
    {
        /// <summary>
        /// Сообщения системы А.
        /// </summary>
        ...
        private readonly ILogger<RandomProducer> logger;
        private readonly IRabbitMqClient _rabbitMqClient;

        public RandomProducer(ILogger<RandomProducer> _logger, IRabbitMqClient rabbitMqClient)
        {
            logger = _logger;
            Messages = new List<MessageModel>();
            _rabbitMqClient = rabbitMqClient;

            //Запуск постоянного процесса отслежующего появление новых сообщений.
                ...
                if(MsgGenCount>0)
                {
                    ...
                    if (NoSendMsg != null)
                    {
                        ...
                        foreach(MessageModel m in NoSendMsg)
                        {
                            ...
                            dto.Prior = m.Prior;
                            //5 кратная попытка отпавить сообщение-иначе исключени.
                            TrySend(dto,5);
        ...
        /// <summary>
        /// Пытает отправить сообщение. Если за указанное количество попыток сообщение не было
